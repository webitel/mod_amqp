    ...
    */
    for (hi = switch_core_hash_first(mod_amqp_globals.logging_hash); hi; hi = switch_core_hash_next(&hi)) {
        ...
        if (logging && switch_log_check_mask(logging->log_level_mask, level)) {
            ...
            snprintf(msg->routing_key, sizeof(msg->routing_key), "%s.%s.%s.%s", switch_core_get_hostname(),
                     node->userdata, switch_log_level2str(node->level), file);

            if (switch_queue_trypush(logging->send_queue, msg) != SWITCH_STATUS_SUCCESS) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
                                  "AMQP logging message queue full. Messages will be dropped!\n");
                switch_safe_free(hi);
                goto done;
            }
        }
...
switch_status_t mod_amqp_logging_create(char *name, switch_xml_t cfg) {
    ...
    if (mod_amqp_connection_open(profile->conn_root, &(profile->conn_active), profile->name, profile->custom_attr) !=
        SWITCH_STATUS_SUCCESS) {
